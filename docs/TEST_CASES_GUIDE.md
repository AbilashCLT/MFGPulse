...
## Overview
62 automated test cases across 9 categories (57 SQL + 5 Python). Run from Admin Panel (Test suite section) or directly in SQL worksheets.

> **Important:** All test files reference `MFGPULSE_DB`.
...
ATP=on_hand-reserved, all 17 parts in ATP view, valid risk/action enums (NO_RISK, ORDER_NOW, EXPEDITE, CRITICAL_SHORTAGE, RESERVED, UNKNOWN), 5 active suppliers, 18 part-supplier mappings.
...
### TC-08: Persona Access (5 tests) — `test_persona_access.py`
6 personas defined, all have copilot, admin restricted to PM+AA, all have wo_po, APP_ADMIN sees all 8 pages.
...
## Acceptance Criteria
Pass when: TC-01 through TC-06 all PASS, TC-07 >=80% PASS, TC-08 all PASS, TC-11 all PASS. Target: >=95% overall.
...
| TC-08 Persona Access | 5 | test_persona_access.py |
