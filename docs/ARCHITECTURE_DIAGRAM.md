...
│  │  │                                                                      │  │
│  │  └─ MAINTENANCE_COPILOT ──────── 7-tool Cortex Agent                   │  │
│  │       ├─ Tool 1: text-to-SQL (via Semantic View)                       │  │
│  │       ├─ Tool 2: RAG search (Cortex Search, 78 docs)                  │  │
│  │       ├─ Tool 3: Fast diagnosis M1-M4 (PREDICT_CORE procedure)        │  │
│  │       ├─ Tool 4: Deep analysis M5-M7 (PREDICT_DEEP procedure)        │  │
│  │       ├─ Tool 5: Monte Carlo simulation (SIMULATE_FAILURE_TWIN)       │  │
│  │       ├─ Tool 6: Work order generation (GENERATE_WORK_ORDER)          │  │
│  │       └─ Tool 7: Purchase order generation (GENERATE_PURCHASE_ORDER)  │  │
│  │                                                                         │  │
│  │  LLM Models Used:                                                       │  │
...
│  │ Tasks / DTs / Streams / Models │ │ 7-tool AI agent              │        │
...
| AGENT | Cortex Agent | 1 | MAINTENANCE_COPILOT (7 tools: maintenance_analytics, maintenance_history, diagnose_asset, deep_analysis, simulate_scenario, generate_work_order, generate_purchase_order) |
...
│   │   └── copilot.py                 # 7-tool Cortex Agent conversational interface
...
│   └── MAINTENANCE_COPILOT_agent.yaml  # Agent definition (7 tools)
│
...
## 8. Why Snowflake — Feature-to-Decision Mapping

Every architectural decision maps to a Snowflake capability. No external services, no glue code, no infrastructure management.

| Architectural Need | Snowflake Feature Used | What It Replaced | Why It Matters |
|---|---|---|---|
| **Auto-refreshing ETL** | Dynamic Tables (14, TARGET_LAG=1h) | Airflow + dbt + cron jobs | Zero ETL scheduling code. Pipeline recalculates automatically when upstream data changes. INCREMENTAL refresh on PHYSICS_COMPOSITE processes only changed rows. |
| **ML model serving** | SQL UDFs (6) + Snowflake.ML (3 native) | MLflow + Flask + model servers | Models execute inside the warehouse — no API latency, no deployment pipeline, no model server to manage. UDFs call inline with DT refresh. |
| **LLM reasoning** | Cortex Complete (llama3.1-70b, llama3.1-8b) | OpenAI API + API keys + rate limiting | One SQL function call. No API keys, no token management, no external network calls. Runs inside Snowflake's trust boundary. |
| **RAG search** | Cortex Search (78 docs) | Pinecone/Weaviate + embedding pipeline | No vector database to manage. INDEX + SEARCH in SQL. Auto-refreshes as maintenance logs grow. |
| **AI agent orchestration** | Cortex Agent (7 tools) | LangChain + custom routing + API gateway | Declarative YAML definition. Agent autonomously chains tools (SQL, search, procedures, UDFs). No framework code. |
| **Semantic query layer** | Semantic View (6 tables, 19 VQRs) | Custom NL-to-SQL pipeline | Verified queries ensure text-to-SQL accuracy. Custom instructions control OEE formula and severity ordering. |
| **Change data capture** | Streams (3) | Debezium + Kafka Connect | Native CDC with zero infrastructure. Streams detect new/changed rows automatically. |
| **Workflow automation** | Tasks (11-task DAG) | Airflow DAG + workers + scheduler | SQL-native DAG with predecessor dependencies. No DAG server, no worker pool, no deployment. |
| **Frontend** | Streamlit in Snowflake (8 pages) | React/Next.js + hosting + auth | No frontend build, no deployment, no auth integration. Runs on Snowflake compute pool with native session auth. |
| **Notifications** | SYSTEM$SEND_SNOWFLAKE_NOTIFICATION | SendGrid/SES + webhook infra | Built-in email delivery. No API keys, no webhook endpoints, no delivery monitoring. |
| **Cost governance** | Resource Monitors (3) + ACCOUNT_USAGE | CloudWatch + custom billing dashboards | Native credit caps with auto-suspend. FinOps dashboard reads ACCOUNT_USAGE views directly. |
| **Security** | Parameterized SQL + persona-gated ops | OAuth + RBAC middleware + audit logging | Session-level auth inherited from Snowflake. No separate auth layer needed. |

**The bottom line:** MFGPulse AI has **zero external dependencies**. Every component — ingestion, transformation, ML, LLM, search, agent, frontend, notifications, scheduling, monitoring — runs natively on Snowflake. A traditional stack would require 8-12 external services to achieve the same functionality.
