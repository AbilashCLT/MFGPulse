...
│  └──────────────────┘  └──────────────────────────────┘   │
│  ┌──────────────────┐  ┌──────────────────────────────┐   │
│  │ diagnose_asset   │  │ simulate_scenario             │   │
│  │ (Fast M1-M4)     │  │ (Monte Carlo what-if)         │   │
│  │ + deep_analysis   │  │ "What if we reduce load 20%?" │   │
│  │ (LLM M5-M7)      │  │                               │   │
│  └──────────────────┘  └──────────────────────────────┘   │
│  ┌──────────────────┐  ┌──────────────────────────────┐   │
│  │ generate_        │  │ generate_purchase_order       │   │
│  │ work_order       │  │ "Order parts for Pump P2"     │   │
│  │ "Create WO for   │  │ Resolves supplier, lead time, │   │
│  │  Fan F1"         │  │ procurement risk               │   │
│  └──────────────────┘  └──────────────────────────────┘   │
│                                                            │
...
- 62 automated tests runnable from Admin Panel (not just developer tests)
...
│  ✓ Cortex Agent with genuine 7-tool orchestration          │
│  ✓ Parameterized SQL, resource monitor, 62 tests (9 test files)  │
...
## Slide 15 — Why Snowflake, Not Traditional Stacks

**Headline**: Zero External Dependencies — Everything Runs Natively

```
┌────────────────────────────────────────────────────────────┐
│                                                            │
│  Capability     Traditional         MFGPulse AI            │
│  ────────────   ──────────────────  ──────────────────     │
│  ETL Pipeline   Airflow + dbt       Dynamic Tables (14)    │
│  ML Serving     MLflow + Flask      SQL UDFs (inline)      │
│  LLM / RAG      OpenAI + Pinecone   Cortex Complete/Search │
│  AI Agent       LangChain + API GW  Cortex Agent (YAML)    │
│  Frontend       React + Vercel      Streamlit in Snowflake │
│  Notifications  SendGrid + webhooks SYSTEM$SEND_SNOWFLAKE  │
│  Scheduling     Cron + Airflow      Tasks (11-task DAG)    │
│  CDC            Debezium + Kafka    Streams (3)            │
│  Cost Mgmt      CloudWatch + custom Resource Monitors      │
│                                                            │
│  Traditional stack: 8-12 services, 5+ API keys,           │
│  separate auth, separate monitoring                        │
│                                                            │
│  MFGPulse AI: 1 platform, 0 external dependencies,        │
│  1 security boundary, 1 cost model                         │
│                                                            │
└────────────────────────────────────────────────────────────┘
```

**Speaker notes**:
- This is the architectural thesis: Snowflake is no longer just a data warehouse — it's a full application platform
- Every component that traditionally requires a separate service (ETL, ML, LLM, search, notifications, frontend) runs natively
- The total infrastructure is 2 X-Small warehouses (auto-suspend 60s) and 1 compute pool — under 1 credit/day operational cost
- No API keys to manage, no services to monitor, no deployments to coordinate — this is what "100% Snowflake-native" actually means

---

## Slide 16 — Live Demo Script (3 Minutes)

**Headline**: See It Running — Prediction to Action in 3 Minutes
...
│  ─────  ───────────────────────────────  ─────────────────  │
│                                                            │
│  0:00   Login as Plant Manager           "Role-based access │
│         → Executive Dashboard             — 6 personas see   │
│                                           different pages"   │
│                                                            │
│  0:30   Show OEE gauge + risk matrix     "Real-time plant   │
│         Point at cost avoided metric      health from 14     │
│         Click "Generate board summary"    Dynamic Tables"    │
│                                                            │
│  1:00   Switch to Copilot                "Natural language   │
│         Type: "What is the most           to 7-tool agent"   │
│         critical asset and why?"                             │
│         (Agent chains diagnose_asset                         │
│          + deep_analysis)                                    │
│                                                            │
│  1:30   Type: "Generate a work order     "Prediction to      │
│         for it"                           action in one       │
│         (Agent calls generate_work_order) sentence"           │
│                                                            │
│  2:00   Switch to Digital Twin           "Monte Carlo sim — │
│         Select the at-risk asset          300 stochastic     │
│         Compare 3 scenarios               paths, 3 scenarios │
│         Show cost delta                   with cost analysis" │
│                                                            │
│  2:30   Switch to Admin Panel            "Built-in FinOps — │
│         Show FinOps credit dashboard      resource monitors  │
│         Show notification settings        + 62 test suite"  │
│                                                            │
│  2:50   Show notification bell           "Closed loop:       │
│         (notification from WO created)    WO created →        │
│                                           notification →      │
│                                           tracking"           │
│                                                            │
│  3:00   Return to Executive Dashboard    "End-to-end:        │
│                                           Sense → Predict →  │
│                                           Act → Track.        │
│                                           100% Snowflake."   │
│                                                            │
│  DEMO ASSET: Use ASSET_001 (Compressor A1) — it typically  │
│  shows bearing_wear with lowest RUL, making the best demo.  │
│  If its RUL is too high, use whichever asset is Critical.   │
│                                                            │
└────────────────────────────────────────────────────────────┘
```

**Speaker notes**:
- Practice this flow 3 times before presenting. The exact click path matters — hesitation kills demo credibility
- If the Copilot is slow (>10s), have a pre-cached session ready and say "Let me show you one I prepared earlier"
- The key "wow moment" is the agent autonomously chaining diagnosis + work order generation from two sentences
- End on the Executive Dashboard so the closing slide transition feels natural
...
│     ─────────────────────────────────────────              │
│                                                            │
│     12 ML models  │  14 Dynamic Tables  │  7-Tool Agent    │
│     8 Pages  │  6 Personas  │  62 Tests  │  0 External    │
│                                                            │
│     Built in ~39 hours with CoCo for 91 credits            │
...
- 11 categories, 62 tests, acceptance criteria (>= 95% pass rate), runnable from Admin Panel
...
│   1 Cortex Agent │ 1 Semantic View │ 62 tests │ 12 docs  │
...
- This demonstrates that a production-grade predictive maintenance platform — with 12 ML models, 14 Dynamic Tables, a 7-tool AI Agent, 8 Streamlit pages, and 62 test cases — can be built efficiently on Snowflake with CoCo as the development copilot
